    ...
    <div class="min-h-screen bg-gray-100 p-3 sm:p-12">
        <div class="mx-auto max-w-lg px-6 py-12 bg-white border-0 shadow-lg rounded sm:rounded-2xl">
            <div class="mx-auto mb-4">
                <h2 class="text-gray-700 text-xl sm:text-3xl font-bold">Edit Salary Slip for {{$currentMonth}}</h2>
                <p class="text-sm sm:text-base text-gray-500">Make slip in each month for staffs</p>
            </div>
            <form id="form" method="POST" action="{{ route('salary-slips.update', $slip->id) }}">
                @csrf
                @method('PATCH')
                ...
                <div class="relative z-0 w-full mb-5">
                    ...
                    <input
                        ...
                        value="{{ old('leave_request') ?? $slip->leave_request }}"
                        required
